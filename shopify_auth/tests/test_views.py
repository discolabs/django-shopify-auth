...
class ViewsTestCase(TestCase):
    ...
    def test_redirect_to_view(self):
        ...
        """
        response = self.client.get(
            "/authenticate/?shop=test.myshopify.com&next=other-view"
        )
        self.assertContains(
            response,
            'window.top.location.href = "https://test.myshopify.com/admin/oauth/authorize',
        )
    ...
    def test_check_cookie(self):
        ...
        self.assertContains(
            response, "window.detect.cookies_third_party.cookies_test_finished(false);"
        )
