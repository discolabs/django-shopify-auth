...
def get_return_address(request):
    return (
        request.session.get(SESSION_REDIRECT_FIELD_NAME)
        or request.GET.get(auth.REDIRECT_FIELD_NAME)
...
def login(request, *args, **kwargs):
    ...
    if shop:
        ...
        if settings.SHOPIFY_APP_IS_EMBEDDED and getattr(
            settings, "SHOPIFY_APP_THIRD_PARTY_COOKIE_CHECK", False
        ):
            response = render(
                request,
                "shopify_auth/check_cookies.html",
                {
...
def check_cookie(request):
    ...
    return HttpResponse(
        "window.detect.cookies_third_party.cookies_test_finished({});".format(
            is_cookie_present
        ),
...
def authenticate(request, *args, **kwargs):
    ...
    if settings.SHOPIFY_APP_DEV_MODE:
        return finalize(
            request, token="00000000000000000000000000000000", *args, **kwargs
        )
    ...
    if shop:
        ...
        permission_url = shopify.Session(
            shop.strip(), getattr(settings, "SHOPIFY_APP_API_VERSION", "unstable")
        ).create_permission_url(scope, redirect_uri)
        ...
        if settings.SHOPIFY_APP_IS_EMBEDDED:
            ...
            return render(
                ...
                {"shop": shop, "redirect_uri": permission_url},
            )
        else:
